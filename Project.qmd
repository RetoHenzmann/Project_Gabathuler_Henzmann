...
# Convert to factors and get to thes same levels
data_geo_join2$transport_mode <- factor(data_geo_join1$transport_mode)
data_geo_join2$travel_mode_det <- factor(data_geo_join1$travel_mode_det)

all_levels <- union(levels(data_geo_join2$transport_mode), levels(data_geo_join2$travel_mode_det))

data_geo_join2$transport_mode <- factor(data_geo_join2$transport_mode, levels = all_levels)
data_geo_join2$travel_mode_det <- factor(data_geo_join2$travel_mode_det, levels  = all_levels)

vdata_geo_join2$transport_mode <- factor(vdata_geo_join1$transport_mode)
vdata_geo_join2$travel_mode_det <- factor(vdata_geo_join1$travel_mode_det)

all_levelsv <- union(levels(vdata_geo_join2$transport_mode), levels(vdata_geo_join2$travel_mode_det))
vdata_geo_join2$transport_mode <- factor(vdata_geo_join2$transport_mode, levels = all_levelsv)
vdata_geo_join2$travel_mode_det <- factor(vdata_geo_join2$travel_mode_det, levels  = all_levelsv)


#Confsionmatrix for cal and train data generat
con_train <- confusionMatrix(data_geo_join1$transport_mode, data_geo_join1$travel_mode_det)


con_val <- confusionMatrix(vdata_geo_join2$transport_mode, vdata_geo_join2$travel_mode_det)


#Visualise Confusionmatrix
...
# Extract the confusion matrix table
conf_val <- as.table(con_val$table)

# Melt the table to long format for ggplot2
conf_val <- melt(conf_val)

# Plot the confusion matrix using ggplot2
ggplot(data = conf_val, aes(x = Reference, y = Prediction, fill = value)) +
    geom_tile(color = "white") +
    scale_fill_gradient(low = "white", high = "blue") +
    geom_text(aes(label = sprintf("%d", value)), vjust = 1) +
    theme_minimal() +
